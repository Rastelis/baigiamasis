...
//create new user
router.post('/vartotojai', addminAuth, upload.none(), passwordValidate, async (req, resp) => {
  try {
...
router.post('/prisijungti', upload.none(), async (req, resp) => {
  try {
    ...
    if (!data)
      return resp.status(401).json('Neteisingai įvesti prisjungimo duomenys arba vartotojas neegzistuoja');
